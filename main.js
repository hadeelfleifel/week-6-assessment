...
function addKeyPowerIndex(arr) {
	var newArr = arr.forEach (function(a,b)
		{
			a[b]=a**2 //if a**2 edit in values without add new key ; b => name of the key ;a[b] => method to add new key; if add return  {0: NaN, a: 12}  
		}
	);
	return arr
}
...
function decreseBy(arr,num) {
	var newArr=arr.map(a=>  a-num); // not reduce coz the result not one value , not filter coz no condition.
	return newArr;
}
...
function nameLongerThan(arr,num) {
	var newArr = arr.filter(a =>
    	(a.name.length > num) // num = 4 ;a.length =5 ; newArr =4 must;
...
function avgLength(arr,key) {
	...
	var newArr=arr.reduce((a,b)=> {
	 return a + b[key].length // a=0 ; fKey = 4 ,sKey=6,... total = 30;
	}, 0);	 
	 return newArr/ arr.length // 30 /5 /6
